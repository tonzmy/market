...
public class PendingOrderViewModel extends ViewModel {
    ...
    private final DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference()
            .child(orderDB).child(key);

    private final FirebaseRepository liveData = new FirebaseRepository(databaseReference);
    private final LiveData<Order> pendingLiveData = Transformations.map(liveData, new Function<DataSnapshot, Order>() {
        @Override
    ...
    private final DatabaseReference dbRoot = FirebaseDatabase.getInstance().getReference().child(orderDB).child(current);

//    private Query recyclerDB = FirebaseDatabase.getInstance().
//            getReference().child(orderDB).orderByChild("status").equalTo("pending");
    private Query recyclerDB = dbRoot.orderByChild("status").equalTo("pending");

    public Query getPendingRecyclerDB() {
        return recyclerDB;
    }

    public LiveData<Order> getPendingLiveData() {
